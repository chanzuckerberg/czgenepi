...
patch -p1 < /aspen/src/py/workflows/nextstrain_run/combine-and-dedup-fastas.py.patch

cp /aspen/src/py/workflows/nextstrain_run/nextstrain_profile/* /ncov/my_profiles/aspen/

# dump the sequences, metadata, and builds.yaml for a run out to disk.
aligned_gisaid_location=$(
    /aspen/.venv/bin/python /aspen/src/py/workflows/nextstrain_run/export.py  \
                            --phylo-run-id "$1"                               \
...
# create the objects
/aspen/.venv/bin/python /aspen/src/py/workflows/nextstrain_run/save.py               \

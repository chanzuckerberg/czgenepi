...
class EnumInsertOp(MigrateOperation):
    ...
    def fully_qualified_tablename(self):
        return (
            "%s.%s" % (self.schema, self.tablename) if self.schema else self.tablename
        )
...
def insert(operations, operation):
    ...
    for value in operation.data:
        operations.execute(
            text(
                f'INSERT INTO {alembic.autogenerate.render._ident(operation.fully_qualified_tablename)} ({alembic.autogenerate.render._ident("item_id")}) VALUES (:vl)'
            ).params({"vl": value})
        )
...
def delete(operations, operation):
    ...
    operations.execute(
        text(
            f'DELETE FROM {alembic.autogenerate.render._ident(operation.fully_qualified_tablename)} WHERE {alembic.autogenerate.render._ident("item_id")} IN :vl'
        ).params({"vl": tuple(operation.data)})
    )


@alembic.autogenerate.render.renderers.dispatch_for(EnumInsertOp)
def render_sync_enum_value_op(autogen_context, op):
    return 'op.enum_insert("{}", {!r}, schema={!r})'.format(
        op.tablename, op.data, op.schema
    )


@alembic.autogenerate.render.renderers.dispatch_for(EnumDeleteOp)
def render_sync_enum_value_op(autogen_context, op):
    return 'op.enum_delete("{}", {!r}, schema={!r})'.format(
        op.tablename, op.data, op.schema
    )

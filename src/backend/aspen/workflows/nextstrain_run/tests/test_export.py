...
def create_test_data(
    ...
    group: Group = group_factory(name=f"testgroup-{tree_type.value}")
    uploaded_by_user: User = user_factory(group, email=f"{tree_type.value}@dh.org", auth0_user_id=tree_type.value)
    session.add(group)
    ...
    gisaid_dump = aligned_gisaid_dump_factory(sequences_s3_key=tree_type.value, metadata_s3_key=tree_type.value).outputs[0]
...
def test_build_config(mocker, session, postgres_database):
    ...
    for tree_type in [TreeType.OVERVIEW, TreeType.NON_CONTEXTUALIZED, TreeType.TARGETED]:

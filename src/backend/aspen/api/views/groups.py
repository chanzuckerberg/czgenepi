...
from aspen.api.schemas.usergroup import GroupMembersResponse, GroupInvitationRequest
...
@router.post("/{group_id}/invitations/", response_model=GroupMembersResponse)
async def invite_group_members(
    group_id: int,
    group_invitation_request: GroupInvitationRequest,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(get_auth_user),
) -> GroupMembersResponse:
    if user.group.id != group_id:

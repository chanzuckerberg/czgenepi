...
def create_auth0_entry(
    ...
    password: str,
    config: Config,
) -> Mapping[str, Union[str, bool, Collection]]:
    ...
    try:
        ...
        sentry_sdk.capture_message(
            f"Error creating auth0 user {e.message} / {e.status_code}"
        )
        raise ex.BadRequestException("User creation failed")
...
def usergroup():
    ...
    if request.method == "POST":
        ...
        if "auth0_user_id" not in new_user_data.keys():
            user_created: Mapping[
                str, Union[str, bool, Collection]
            ] = create_auth0_entry(
                new_user_data["name"],
                new_user_data["email"],
                "pwd",

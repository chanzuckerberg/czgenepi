...
class UploadSampleStep extends React.Component {
    ...
    validateAddedSamples = async (samples, sampleType) => {
        ...
        const sampleFileNames = flatten(
            map((sample) => keys(sample.files), samples)
        );
        ...
        validatedSamples = validatedSamples.map((sample) =>
            set(
                "files",
                pickBy(
                    (_file, fileName) => fileNamesValidMap[fileName],
                    sample.files
                ),
                sample
            )
        ...
        validatedSamples = validatedSamples.map((sample) =>
            set(
                ...
                filter(
                    (inputFileAttributes) =>
                        fileNamesValidMap[inputFileAttributes.source],
                    sample.input_files_attributes
                ),
                sample
            )
    ...
    handleSamplesAdd = async (newSamples, sampleType) => {
        ...
        const {
            ...
            removedLocalFiles,
        } = await this.validateAddedSamples(
            newSamplesWithSelectIds,
            sampleType
        );
            ...
            case LOCAL_UPLOAD:
                return (
                    <LocalSampleFileUpload
                        onChange={(samples) =>
                            this.handleSamplesAdd(samples, "local")
                        }
                        project={selectedProject}
            ...
            case REMOTE_UPLOAD:
                return (
                    <RemoteSampleFileUpload
                        project={selectedProject}
                        onChange={(samples) =>
                            this.handleSamplesAdd(samples, "remote")
                        }
            ...
            case BASESPACE_UPLOAD:
                return (
                    <BasespaceSampleImport
                        project={selectedProject}
                        onChange={(samples) =>
                            this.handleSamplesAdd(samples, "basespace")
                        }
                        accessToken={basespaceAccessToken}
                        onAccessTokenChange={
                            this.handleBasespaceAccessTokenChange
                        }
                        basespaceClientId={this.props.basespaceClientId}
                        basespaceOauthRedirectUri={
                            this.props.basespaceOauthRedirectUri
                        }
                        onNoProject={this.handleNoProject}
                    />
                );
            ...
            >
                <div className={cs.flexContent}>
                    ...
                    {this.state.currentTab === LOCAL_UPLOAD &&
                        this.state.removedLocalFiles.length > 0 && (
                            <IssueGroup
                                ...
                                rows={this.state.removedLocalFiles.map(
                                    (file) => [file]
                                )}
                                type="warning"
                            />
                    ...
                    <SampleUploadTable
                        ...
                        selectedSampleIds={this.getSelectedSampleIds(
                            currentTab
                        )}
                ...
                <div className={cs.controls}>
                    {readyForBasespaceAuth && (
                        <div className={cs.helpText}>
                            Please authorize IDseq to fetch your selected
                            samples from Basespace.
                        </div>
                    )}
                    <PrimaryButton

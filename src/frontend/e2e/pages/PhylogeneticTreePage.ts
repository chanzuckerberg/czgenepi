...
export class PhylogeneticTreePage {
    ...
    constructor(page: Page){
        ...
        this.exceedNumberOfCharsErrorMsg = page.locator("//div[text()='Name exceeds the 128 character limit.']");
        this.sampleWarningMessage = page.locator("//div[contains(@class,'MuiDialog-paperScrollPaper')]/descendant::div[contains(@class,'MuiAlert-messag')]/descendant::span[1]");
    }
        ...
        if (treeInfo.treeType.length > 0){
            for(let i = 0; i < types.length; i++){
               ...
               //    await this.page.locator('//input[contains(@value,"${VAR}")]'.replace('${VAR}',treeInfo.treeType)).click();
                    await this.page.locator(`.MuiFormGroup-root > label:nth-of-type(${i+1}) > span > input`).click();
                }
            }
        }

...
export class UploadPage {
    ...
    constructor(page: Page){
        ...
        this.sampleCollectionLocation = "//div[div[text()='${VAR}']]/following-sibling::div/descendant::button[@label='Search For Location']";
        this.sampleSequencingDate = "//div[div[text()='${VAR}']]/following-sibling::div/descendant::input[@name='sequencingDate']";
        this.samplePrivateSwitch = "//div[div[text()='${VAR}']]/following-sibling::div/descendant::input[@name='keepPrivate']";
    ...
    async clickContinue(){
        ...
        await this.page.waitForSelector("form div[role='table'] > .MuiTableBody-root");
    }
    ...
    async fillSampleInfo(sampleName: string, sampleData: SampleData ){
        await this.page.locator(this.samplePrivateId.replace('${VAR}',sampleName)).click();
        while(await this.page.locator(this.samplePrivateId.replace('${VAR}',sampleName)).getAttribute("value") !== ''){
            await this.page.keyboard.press('Backspace');
            await this.page.keyboard.press('Delete');
        }
       await this.page.locator(this.samplePrivateId.replace('${VAR}',sampleName)).type(sampleData.privateId);
       ...
       await this.page.locator(this.sampleSequencingDate.replace('${VAR}',sampleName)).type(sampleData.sequencingDate);
        if(sampleData.isPrivate === true){
            await this.page.locator(this.samplePrivateSwitch.replace('${VAR}',sampleName)).click();
        }

    }

    async getCollectionDateInvalidFormatMessage(sampleName: string): Promise<string|null>{
        return this.page.locator(this.collectionDateInvalidFormatMsg.replace('${VAR}',sampleName)).textContent();
    }

    async getsequencingDateInvalidFormatMessage(sampleName: string): Promise<string|null>{
        return this.page.locator(this.sequencingDateInvalidFormatMsg.replace('${VAR}',sampleName)).textContent();
    }

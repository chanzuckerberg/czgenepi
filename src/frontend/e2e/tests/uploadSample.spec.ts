...
test.describe("Filter Test Suite",()=>{
    ...
    test('upload extension files test',async ()=>{
        ...
        await expect(uploadPage.importedFileNameList).toHaveText(await uploadPage.getImportedFileNameList());
    ...
    test('sample form test1', async({page})=>{
        ...
        await expect(await uploadPage.getCollectionDateInvalidFormatMessage('hCoV-19/USA/CA-CCC_Ex8')).toBe('Update format to YYYY-MM-DD')
        await expect(await uploadPage.getsequencingDateInvalidFormatMessage('hCoV-19/USA/CA-CCC_Ex8')).toBe('Update format to YYYY-MM-DD')
        await uploadPage.fillSampleInfo('hCoV-19/USA/CA-CCC_Ex9',sampleData2);
        await expect(await uploadPage.getCollectionDateInvalidFormatMessage('hCoV-19/USA/CA-CCC_Ex9')).toBe('Update format to YYYY-MM-DD')
        await expect(await uploadPage.getsequencingDateInvalidFormatMessage('hCoV-19/USA/CA-CCC_Ex9')).toBe('Update format to YYYY-MM-DD')
